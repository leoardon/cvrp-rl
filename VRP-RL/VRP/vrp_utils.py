...
def create_VRP_dataset(
    ...
    if os.path.exists(fname):
        ...
        d = rnd.randint(1,max_demand,[n_problems,n_nodes,1])
...
class DataGenerator(object):
    def __init__(self, 
        ...
        self.test_data = create_VRP_dataset(self.n_problems,args['n_cust'],'./data',
            max_demand=args['demand_max'], seed = args['random_seed']+1,data_type='test')
...
#         self.input_pnt = tf.tile(self.input_pnt, [self.beam_width,1,1])
    ...
    def step(self,
        ...
        if "LEO_UPDATE" in os.environ:
            # mask when demand exceeds current load
            self.mask += tf.concat([
                tf.cast(tf.greater(self.demand, tf.tile(tf.expand_dims(self.load, 1), [1,self.n_cust+1])), tf.float32)[:,:-1],
